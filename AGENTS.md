...
- `AGENTS.md` -- shared project instructions and managed RPI policy
- `CLAUDE.md` -- project facts imported by Claude Code through this file
- `.agents/skills/*/SKILL.md` -- Codex workflow and domain packages
- `.claude/skills/*/SKILL.md` -- Claude Code workflow and domain packages
- `.rpi/rules/*.md` -- shared conditional rules
...
When the user invokes an RPI workflow such as `/rpi-research`,
`/rpi-plan`, `/rpi-implement`, `/rpi-validate`, `/rpi-pre-launch`,
`/rpi-update-docs`, `/rpi-release`, `/rpi-fix-ci`,
`/rpi-describe-pr`, or `/rpi-status`:

1. Claude Code reads `.claude/skills/<workflow>/SKILL.md`; Codex reads the
   matching `.agents/skills/<workflow>/SKILL.md`.
2. Read the selected skill and its required references completely before acting.
3. Follow it as the workflow spec for this task.
4. Keep all outputs in the repository locations required by that skill.

The retained `/release` command is a legacy explicit-only alias. Prefer
`/rpi-release`. If a workflow references a plan path or research path, read that
document fully before doing anything else.

## Harness Translation

Translate harness-specific operations to the closest equivalent behavior:
...
Always follow the managed instruction blocks below. Load `.rpi/rules/` through
the conditional rule map:

- The embedded RPI details and push accountability blocks always apply.
- Read a conditional rule when the task or edited path matches its map entry.
- A workflow skill governs its process; the rule governs local constraints.

## Skills

Treat `.agents/skills/*/SKILL.md` and `.claude/skills/*/SKILL.md` as the native
on-demand packages for their respective harnesses:
